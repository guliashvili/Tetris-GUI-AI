...
public class Piece {
	...
	*/
	...
	private void computeBody(TPoint[] points){
	...
	private void computeSizes(){
		width=height=0;
		for(TPoint elem : body){
			width = Math.max(width, elem.x);
			height = Math.max(height, elem.y);
		}
		width++;
		height++;
	}
	private void normalize(){
		int mnX=body[0].x,mnY=body[0].y;
		
		for(TPoint elem : body){			
			mnX = Math.min(mnX, elem.x);
			mnY = Math.min(mnY, elem.y);
		}
		
		for(int i = 0;i < body.length;i++)
			body[i] = new TPoint(body[i].x-mnX,body[i].y-mnY);
	...
	private void computeSkirt(){
		skirt = new int[width];
		for(int i = 0; i < width; i++) skirt[i]=-1;
		
		for(TPoint elem : body)
			skirt[elem.x]=(skirt[elem.x]==-1)?elem.y:Math.min(skirt[elem.x], elem.y);
	...
	*/
	...
	public Piece computeNextRotation() {
		...
		for(int i = 0; i < points.length; i++)
			points[i] = new TPoint(height-body[i].y-1 ,body[i].x);
	...
	*/
	public boolean equals(Object obj) {
		...
		//if(this.width != other.width || this.height != other.height || this.body.length != other.body.length || this.hashCode()!=other.hashCode()) return false;
	...
	*/
	/*
	 ...
	 to the first piece.
	*/
	private static Piece makeFastRotations(Piece in) {
		Piece root = new Piece(in.body);
		Piece cur = root;
		cur.next = cur;
		...
		}while(!cur.next.equals((Object)root));
